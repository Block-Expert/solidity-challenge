...
Bank.withdraw() is vulnerable to the reenctrancy attack
So a hacker tries to drain Ether from Bank.
But actually the reentrancy exploit is a bait for hackers
By deploying Bank with HnoeyPot in place of the logger, this contract becomes
a trap for hackers. Let's see how.

1. Alice deploy HoneyPot
2. Alice deploys Bank with the address of HoneyPot
3. Alice deposits 1 Ether into Bank
4. Eve discovers the reentrancy exploit in Bank.withraw and decides to hack it.
5. Eve deploys Attack with address of Bank
